      ...
      <ScrollReveal direction="down">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <motion.div variants={fadeUp} className="max-w-2xl">
            <h1 className="text-4xl md:text-5xl font-display font-bold text-text tracking-tight mb-3">
              Soil Analysis Portal
            </h1>
            <p className="text-text-muted font-medium text-lg leading-relaxed max-w-lg">
              Upload reports or enter data to get AI-powered insights.
            </p>
          </motion.div>

          {/* Mode Toggles */}
          <motion.div variants={fadeUp}>
            <div className="flex bg-surface-2/40 backdrop-blur-md rounded-full p-1 border border-white/5 shadow-2xl">
              {([
                { id: 'upload', icon: Upload,  label: 'UPLOAD FILE'   },
                { id: 'ocr',    icon: Camera,  label: 'OCR IMAGE'     },
                { id: 'manual', icon: Edit3,   label: 'MANUAL ENTRY'  },
              ] as const).map((m) => (
                <button
                  key={m.id}
                  onClick={() => setMode(m.id)}
                  className={cn(
                    'flex items-center gap-2.5 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-500',
                    mode === m.id
                      ? 'bg-white text-bg shadow-xl scale-[1.02]'
                      : 'text-text-muted hover:text-text hover:bg-white/5'
                  )}
                >
                  <m.icon size={14} className={mode === m.id ? 'text-bg' : 'text-text-muted'} />
                  {m.label}
                </button>
              ))}
            </div>
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 space-y-4">
                <div className={cn(
                  'w-16 h-16 mx-auto rounded-2xl glass flex items-center justify-center transition-all duration-500 border',
                  'group-hover:scale-110 group-hover:bg-white/10',
                  isDragActive ? 'border-success text-success' : 'border-white/10 text-white'
                )}>
                  {uploading
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black text-white/40 mt-3 uppercase tracking-[0.2em]">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h3 className="text-2xl font-display font-bold text-white tracking-tight">
            Results for ({soilReports.length})
          </h3>
        </div>
          ...
          <AnimatePresence>
            {soilReports.map((report, i) => (
              <ScrollReveal key={report.id} direction={i % 2 === 0 ? 'left' : 'right'} delay={0.05}>
                ...
                >
                  <div className="flex items-center gap-6 relative z-10 px-2 py-1">
                    {/* Status checkmark */}
                    <div className="shrink-0">
                      <div className="w-12 h-12 rounded-full border-2 border-white/10 flex items-center justify-center text-white/40">
                         <CheckCircle size={24} />
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-lg font-medium text-white tracking-tight truncate">
                          {report.fileName}
                        </p>
                      </div>
                      <p className="text-xs font-medium text-white/30 mt-1 uppercase tracking-wider">
                        Upload Data: {report.uploadDate}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-8">
                       <div className="flex items-center gap-2">
                         <span className="text-[11px] font-black text-white/80 uppercase tracking-widest">Active</span>
