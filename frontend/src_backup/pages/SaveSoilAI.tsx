...
export default function SaveSoilAI() {
  ...
  const stats = [
    { label: t('saveSoil.healthScore'), value: '82%', icon: LineChart, color: 'text-primary' },
    { label: t('saveSoil.carbonReduction'), value: '35%', icon: Cloud, color: 'text-info' },
    { label: t('saveSoil.waterSaved'), value: '1,200L', icon: Droplets, color: 'text-info' },
    { label: t('saveSoil.sustainabilityScore'), value: '88%', icon: Leaf, color: 'text-primary' }
  ];
  ...
  const handleAskAI = () => {
    ...
    setPendingChatQuery(`Regarding my soil health and sustainability: ${query}`);
    setPanelOpen(true);
    setQuery('');
  };
  ...
  return (
    <div className="space-y-12 pb-20">
          ...
          <div className="relative z-10 w-full p-8 md:p-14 flex flex-col md:flex-row justify-between items-center gap-10">
            <div className="max-w-xl space-y-8">
              <motion.div 
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-dim backdrop-blur-md border border-primary/30 text-primary text-[10px] font-black uppercase tracking-widest shadow-xl"
              >
              ...
              <motion.button
                whileHover={{ scale: 1.05, backgroundColor: 'var(--teal-hover)' }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setIsModalOpen(true)}
                className="flex items-center gap-3 px-10 py-5 bg-primary text-bg rounded-2xl font-black text-sm uppercase tracking-widest shadow-2xl shadow-primary/30 transition-all border border-primary/50"
              >
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-primary-dim flex items-center justify-center text-primary border border-primary/40">
                <Sparkles size={32} />
              </div>
              <div className="text-center">
                <p className="text-[10px] font-black text-primary/80 uppercase tracking-widest mb-1">Global Impact</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <ScrollReveal direction={i % 2 === 0 ? 'left' : 'right'} delay={i * 0.1} key={i}>
            <div className="p-6 glass border border-white/10 rounded-[32px] group hover:border-white/20 transition-all">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {features.map((feature, i) => (
          <ScrollReveal key={i} direction={i % 2 === 0 ? 'left' : 'right'} delay={0.1}>
            <div className="group relative h-full p-8 glass border border-white/10 rounded-[40px] hover:border-primary/30 transition-all overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
                <feature.icon size={120} />
              </div>
              <div className="relative z-10 space-y-6">
                <div className="w-14 h-14 rounded-2xl bg-primary-dim flex items-center justify-center text-primary">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <ScrollReveal direction="left" className="lg:col-span-2">
          <div className="glass border border-white/10 rounded-[40px] p-8 space-y-6 h-full">
            ...
            <div className="relative">
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-medium focus:outline-none focus:border-primary/50 transition-all placeholder:text-text-muted text-white"
              />
              <button 
                onClick={handleAskAI}
                className="absolute right-2 top-2 p-3 bg-primary text-bg rounded-xl transition-all hover:scale-105 active:scale-95"
              >
                <Send size={18} />
              </button>
            </div>
        ...
        <ScrollReveal direction="right">
          <div className="flex flex-col justify-center items-center glass border border-white/10 rounded-[40px] p-8 text-center space-y-8 relative overflow-hidden h-full">
            <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-accent/20 animate-pulse pointer-events-none" />
            <div className="relative z-10 space-y-6 flex flex-col items-center">
              <div className="w-20 h-20 rounded-[32px] bg-primary shadow-2xl shadow-primary/30 flex items-center justify-center text-bg mb-2">
                <Leaf size={40} />
              </div>
              <h3 className="text-2xl font-black tracking-tight leading-tight text-white">Ready to Make<br/>an Impact?</h3>
              <button
                onClick={() => setIsModalOpen(true)}
                className="w-full py-5 bg-bg text-primary border-2 border-primary rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-primary hover:text-bg transition-all duration-300 shadow-xl"
              >
            ...
            >
              ...
              <div className="space-y-8">
                <div className="text-center space-y-4">
                  <div className="w-16 h-16 rounded-2xl bg-primary-dim flex items-center justify-center text-primary mx-auto">
                    <ShieldCheck size={32} />
                  </div>
                ...
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="w-full py-5 bg-primary text-bg rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-primary/20"
                >
