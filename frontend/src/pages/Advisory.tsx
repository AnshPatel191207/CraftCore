...
import {
  CloudRain, Bug, Sprout, FlaskConical, TrendingUp,
...
const CATEGORIES = ['all', 'weather', 'pest', 'crop', 'soil', 'market'] as const;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        !advisory.isRead
          ? 'border-teal-500/30 bg-teal-500/5 shadow-[0_0_40px_rgba(17,209,177,0.05)]'
    ...
    >
      ...
      <div className="p-8 cursor-pointer relative z-10" onClick={handleExpand}>
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1 min-w-0 pt-1">
            <div className="flex items-start justify-between gap-4">
              ...
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  <h4 className="text-2xl font-display font-black text-text tracking-tight leading-tight uppercase">
                    {advisory.title}
                  </h4>
                  {!advisory.isRead && (
                    <span className="flex h-2.5 w-2.5 relative">
                      <span className="animate-ping absolute inline-flex h-2.5 w-2.5 rounded-full bg-teal-500 opacity-75" />
                ...
                <div className="flex items-center gap-3 mt-3 flex-wrap">
                  <span className={cn(
                    'text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest leading-none border',
                    cat.color
                  )}>
                    {cat.label}
                  </span>
                  <span className={cn(
                    'text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest leading-none flex items-center gap-2',
                    sev.color
                  )}>
                    <SevIcon size={12} />
                    {sev.label}
                  </span>
                  <span className="text-[10px] font-black text-text-muted uppercase tracking-widest bg-white/5 px-3 py-1.5 rounded-full">
                    {advisory.date}
                  </span>
                </div>
              </div>
            ...
            {!expanded && (
              <p className="text-sm text-text-muted mt-4 font-bold leading-relaxed italic opacity-70 truncate">
                {advisory.summary}
              </p>
            )}
          </div>
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="space-y-3">
                    {advisory.actionItems.map((item, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-3 text-xs font-black text-text bg-surface-2 p-4 rounded-2xl border border-white/5"
                      >
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp} className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-5xl font-display font-black text-text tracking-tighter mb-4 text-glow">
            Intelligence Nexus
          </h1>
          <p className="text-text-muted font-bold uppercase tracking-[0.2em] text-[10px]">
            Strategic Recommendations • Real-time Risk Mitigation
          </p>
        </div>
      ...
      <motion.div variants={fadeUp} className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Object.entries(categoryConfig).map(([key, config]) => {
            ...
            const iconColor = config.color.split(' ')[0];
            return (
              <motion.div
                key={key}
                ...
                className={cn(
                  ...
                  filter === key
                    ? 'bg-white/[0.05] border-teal-500/40'
              ...
              >
                ...
                <Icon size={24} className={cn('mx-auto mb-3', iconColor)} />
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          ...
          <span className="text-[10px] font-black text-text-muted opacity-50 uppercase tracking-widest">
            {filtered.length} active notifications
          </span>
        </div>
        ...
        <div className="space-y-6">
          ...
          {filtered.length === 0 && (
            ...
            >
              ...
              <p className="text-sm font-black text-text-muted uppercase tracking-widest leading-relaxed">
                No strategic advisories detected <br /> for this protocol.
              </p>
            </motion.div>
          )}
        </div>
