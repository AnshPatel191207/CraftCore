                     ...
                     <AnimatePresence mode="wait">
                        {aiResponse ? (
                           <motion.div
                              ...
                              exit={{ opacity: 0, y: -10 }}
                              className="p-8 rounded-[32px] bg-indigo-500/10 border border-indigo-500/20 relative"
                           >
                              <div className="flex gap-4">
                                 <div className="w-8 h-8 rounded-full bg-indigo-500 flex-shrink-0 animate-pulse" />
                                 <p className="text-lg font-bold text-white leading-relaxed italic">"{aiResponse}"</p>
                              </div>
                           </motion.div>
                        ) : (
                           <div className="grid grid-cols-2 gap-4">
                              {['Market trends for Wheat?', 'Best crop for high profit?', 'When to sell Maize?'].map((q, i) => (
                                 <button
                                    key={i}
                                    onClick={() => { setQuery(q); }}
                                    className="p-4 rounded-2xl bg-white/5 border border-white/5 text-left text-xs font-bold text-white/40 hover:bg-white/10 hover:text-white transition-all"
                                 >
                                    {q}
                                 </button>
                              ))}
